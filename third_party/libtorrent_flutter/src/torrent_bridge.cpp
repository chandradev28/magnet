...
TORRENT_API void lt_stop_stream(lt_session_t session, lt_stream_id sid) {
    ...
    // Stream-only torrents must remain valid after a player closes so Retry,
    // switching files, and reopening the same magnet can reuse its metadata.
    // The app's explicit removeTorrent/disposeTorrent APIs are responsible for
    // deleting the torrent. Stopping a stream only pauses its data priorities.
    ...
    if (ephemeral) {
        try {
            ...
            if (ti2) {
                const int nf = ti2->files().num_files();
                std::vector<lt::download_priority_t> p(
                    (size_t)nf, lt::dont_download);
